...
export function gen_flow(): FlowGraph {
    ...
    const if_true = builder.add_op('control_wait', { args: [ 1 ] });
    const if_false = builder.add_op('control_wait', { args: [ 2 ] });
    trigger.then_id(builder.add_if(if_true, if_false, { cond: [cond, 0] }))

    const branch3 = builder.add_op('control_wait', { args: [ 3 ]
                                                      });

    const branch4 = builder.add_op('control_wait', { id: 'branch2',
                                                        args: [ 4 ]
                                                      });
    ...
    const fork_joiner = builder.add_trigger('trigger_when_first_completed', {args: [[branch3, 'pulse'], [ branch4, 'pulse']]});

    const if_joiner = builder.add_trigger('trigger_when_first_completed', {args: [[fork_joiner, 'pulse'], [ if_false, 'pulse']]});
    if_joiner.then(f => f.add_op('control_wait', { args: [ 9 ] }));

    const graph = builder.build();
    return graph;
}

describe('Flow-21-07: IF then fork, close with IF merging (wait FIRST, long form).', () => {

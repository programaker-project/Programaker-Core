...
export function gen_flow(options?: { source_id?: string }): FlowGraph {
    ...
    const operation = builder.add_op('control_wait_for_next_value', { args: ["sample"]
                                                                    });
...
describe('Flow-27-05: Wait for value does not accept a constant input.', () => {
    it('Validation should FAIL', async () => {
        expect(() => validate(gen_flow()))
            .toThrowError(/^ValidationError:.*Wait for value does not accept a constant as input.*/i)
    });
});

...
export function gen_flow(): FlowGraph {
    ...
    // Join branch 1 and 2
    const joiner12 = builder.add_trigger('trigger_when_all_completed', {args: [[ branch1, 'pulse' ], [branch4, 'pulse']]})
        .then(f => f.add_op('op_wait_seconds', { args: [ 12 ] }));

    // Join branch 2 and 3
    const joiner23 = builder.add_trigger('trigger_when_all_completed', {args: [[ branch5, 'pulse' ], [branch3, 'pulse']]})
        .then(f => f.add_op('op_wait_seconds', { args: [ 23 ] }));
